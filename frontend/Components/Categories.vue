...
<div>
    ...
    <div>
        <ul>
            <li v-for="category in categories">{{ category.name }} <button style="color: green" @click="editCategory(category._id)">Edit</button> <button style="color: red" v-on:click="deleteCategory(category._id)">Delete</button></li>
        </ul>
    </div>
</div>
</template>
...
export default {
  ...
  methods: {
    addCategory () {
        axios.post('http://localhost:8000/api/category', {categoryName: this.categoryName}).then(function(response){
            this.fetchCategory()
            this.categoryName = ''
        }.bind(this))
    },
    ...
    updateCategory () {
        axios.patch('http://localhost:8000/api/category/',{id: this.updatingCategoryId, categoryName: this.categoryName}).then(function(response){
            this.fetchCategory()
            this.categoryName = ''
